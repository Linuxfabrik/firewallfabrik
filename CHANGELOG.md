...
**Highlights:** A large-scale correctness pass over both compilers. A whole class of rules that silently compiled into something other than what the GUI shows is now either compiled correctly or reported at compile time, instead of matching every address, every service, or nothing at all. Two of those are worth naming: a routing rule whose destination is a group, an address range or a host installed a default route out of the gateway meant for that one destination, sending all traffic the wrong way; and a block list whose file turned out to be empty or unreadable matched every address there is, so the rule blocked or permitted everything. Two more from the same pass: a rule the compiler had just reported as impossible to compile was installed anyway, without the condition it could not express, so "accept established connections to these ports" became "accept everything to those ports"; and on nftables a rule asking for policy routing was compiled into a rule that matched a packet mark and did nothing, silently dropping the route it was written for, where iptables refused the same rule by name. Two more from the newer nftables compiler are worth naming: a rule whose action could not be written out - a Branch pointing at a rule set nftables cannot jump to, or a Custom action - was installed with all of its conditions and no action at all, so it counted the packets and left the decision to whatever rule came next while the activation reported success; and a firewall reading a block list from a file only found out that it could not read the file after it had already replaced its running ruleset, leaving the new one in place with an empty list, which blocks nothing. Two from this round belong in the same list: a firewall with "Always permit SSH access from the management workstation" never got that rule into its ruleset - only into the `block` and `stop` commands - so activating a policy that does not itself permit SSH cut the session doing the activating and left the box unreachable, which is the one thing the option exists to prevent; and on iptables the name of a rule set, a branch or an interface went into the generated script as a plain word, so a name holding shell syntax ran as a command on the firewall, as root, at the moment every chain is already set to drop. One more from this round belongs on the list: a rule limited to a calendar window - "block this until the end of the year" - was compiled with only the time of day and the weekdays the time object names, so it ran every day and never stopped, on both platforms; and on iptables a rule branching into a rule set the firewall does not compile was turned into a jump to an empty chain, so the branch quietly did nothing while the activation reported success. Three from this round name the same shape once more: a DNS Name or Address Table object created in FirewallFabrik kept what the admin typed in a place the compilers never read, so the object resolved its own name or, for a block list, nothing at all - a rule written against a list of hosts stopped none of them; on nftables a rule written for "any interface except these two" also matched the loopback and every interface the firewall does not know about, where iptables read it as the firewall's own remaining interfaces; and on nftables a rule whose source or destination is "not these addresses" was installed for forwarded traffic only, leaving the firewall's own traffic outside a rule written to cover it. Four from this round belong on the list as well: a rule written for "any interface except these two" also produced rules on interfaces the administrator had marked unprotected, on the ports of a bridge and on cluster interfaces, none of which such a rule can ever match, so a Deny let through exactly the traffic it was written to stop; ticking "MAC address matching" on a host changed nothing at all, so a host meant to be reachable only from its own MAC address was matched on its IP address alone; a compile the compiler refused outright wrote no script, said nothing and reported success, so a scheduled or scripted run left the firewall on its previous rules without anyone noticing; and saving a policy and opening it again lost the Tag Service of every rule with the Tag action, so all of them were reported as having no mark and left out of the compiled script - and with them every routing decision and traffic class keyed on one. Two from this round join them, both on nftables and both about a rule written as "anything except this": a rule whose service is negated was compiled as a rule about that one protocol, so "deny anything but http" denied nothing but TCP and let every UDP and ICMP packet through, and a NAT rule written the same way let them cross untranslated; and where the excluded service was a whole protocol ("anything but TCP"), the negation was dropped altogether and the rule came out as the exact rule it was the negation of. Three from this round belong in the same list. A rule naming the firewall object - "permit SSH to the firewall" - was shortened to a rule with no address at all whenever the script also adds addresses for NAT, so it permitted SSH to every translated address as well: a hole opened by an optimisation, and the one Firewall Builder guards against by name. A rule branching into a rule set that marks packets or assigns a traffic class was installed only where those rules are not, so it jumped into an empty chain and nothing downstream ever saw the traffic the policy says is marked. And on a bridging firewall a rule about the firewall's own address covered only traffic arriving on a routing interface, so the same rule that permits SSH dropped it whenever it came in over a bridge. Every generated nftables ruleset loads and every generated script parses, both for the first time across the whole test corpus. On iptables, "Clear all rules" really clears them again, and dual-stack firewalls no longer carry a copy of every IPv6-only rule in their IPv4 chains. Four more from this round belong on the list. A rule of a branch rule set written for one direction applied in both, because the chain of a branch rule set is reached from the input chain and the output chain alike - an Accept written for outgoing traffic opened the incoming side with it. A rule naming a host or firewall whose address comes from DHCP or PPP matched the placeholder address 0.0.0.0 instead of the address the machine actually has, so an anti-spoofing rule written about the firewall never matched it. Two routing rules aiming at the same destination were installed as two competing routes, of which the kernel takes the first and refuses the second, instead of the one route with two next hops the administrator asked for. And a firewall whose rule sets are all branches compiled to a script that installs no filtering at all and reported success. Three from this round belong on the list, and all three are about which traffic a rule was ever asked about. A rule naming a broadcast or a multicast address - the addresses a routing protocol, a DHCP client and neighbour discovery talk to - was installed only on the forwarding path, where such a packet never goes, so a rule written to permit one permitted nothing and a Deny written the same way stopped nothing. A rule naming an address the firewall carries, or a network the firewall is on, was treated as a rule about some other machine unless the address happened to be taken straight from the interface, so it did not cover the firewall's own traffic - "permit SSH to 198.51.100.1" landed where it could not match. And the kernel-hardening settings did not take effect: reverse path filtering, ICMP redirects, source routing and martian logging exist once per interface next to the machine-wide copy, and the script wrote only the machine-wide one, so switching reverse path filtering or ICMP redirects *off* changed nothing on any current distribution. Three from this round belong on the list, and all three are about a rule quietly ending up somewhere it cannot work. A machine that appears in the object tree twice - once as the firewall object and once as a host, in another library or in a rule written before the firewall object existed - was read as some other machine, so every rule naming it went onto the forwarding path, where the firewall's own traffic never goes: an Accept written that way permitted nothing and a Deny stopped nothing. On nftables a rule written outbound whose destination is a network - a multicast group, for instance - was thrown away without a word, so "permit OSPF out of eth1" was in the iptables ruleset and missing from the nftables one. And a rule matching a host by IP *and* MAC address was left out entirely wherever the packet no longer carries an ethernet header, instead of matching on the IP address alone the way Firewall Builder does it. Three from this round belong on the list. A setting a data file writes across several lines - which Firewall Builder does, and reads back the same way - was read as its own opposite, so a rule set marked "holds mangle rules only" had its whole policy installed in the filter table instead: it duplicated the filtering and set none of the packet marks it was written for. A firewall whose file does not carry "Configure interfaces", "Add virtual addresses for NAT" or "Verify interfaces" - every cluster, and every file older than those settings - got a script that silently did none of the three, because both compilers substituted "off" for the default the settings dialog shows; and on nftables the same settings were read from the iptables schema, so the two platforms could disagree about the same firewall. And an object imported from a Firewall Builder file lost its tags, so the tag field of every editor came up empty and a Dynamic Group selecting on a tag matched nothing, taking every rule that names it out of the compiled script. One from this round is about what an editor lets through rather than about what a compiler does with it: a netmask that looked like a bit length but was not - `+8`, say - was taken by the address editors and written to the file as typed, where neither compiler could read it back and both dropped it, so a rule written for a whole network was installed as a rule about a single address, silently and on both platforms. Most of this lands in cases a typical policy never reaches: recompiling 335 firewalls from real configurations leaves the iptables rules unchanged on all but one of them, while the nftables side, being the younger compiler, changes on roughly one in ten. Re-compile and review your rulesets after updating, and read the breaking change below before you do.
...
* The `--xt` option of `fwf-ipt` is gone. It promised to treat fatal errors as warnings, which is what both compilers do anyway, and it was read by nobody.
...
* Editor: a netmask the compilers cannot read is refused where it is typed instead of being stored. A value that looked like a bit length but was not, `+8` for instance, passed the check and went into the data file as typed, and both compilers then dropped the netmask without a word - so a rule written for a whole network was installed as a rule about a single address. The address editors and the new-host wizard now refuse what Firewall Builder refuses: a netmask with zeroes in the middle, a length outside the address family, and a `0.0.0.0` on a network object, which matches every address there is. A netmask typed as a length is stored the way Firewall Builder stores it ([#154](https://github.com/Linuxfabrik/firewallfabrik/issues/154)).

* Compiler (iptables, nftables): the rule that logs a packet in an invalid connection state writes the same prefix Firewall Builder writes, `INVALID state -- DENY `. It said only `INVALID `, so log alerting keyed on the old prefix stopped matching after a migration and the line no longer said what the firewall did with the packet.
...
* Compiler (iptables, nftables): a firewall setting the data file does not carry takes the default the settings dialog shows. Both compilers read a handful of settings straight out of the stored data with a fallback of their own, and that fallback said "off" where the default is "on" - so "Configure interfaces", "Add virtual addresses for NAT" and "Verify interfaces" did nothing on a firewall whose file predates them, and on every cluster. On nftables the default came from the iptables settings, because a firewall imported from Firewall Builder says "iptables" whatever it is compiled with; "Configure interfaces" was also declared off there while iptables has it on, so the same firewall behaved differently on the two platforms.

* Compiler (iptables, nftables): a setting stored across several lines in the data file is read the way Firewall Builder reads it. A "yes" that a file writes on a line of its own was read as "no" wherever the value is compared and as "yes" wherever it is only tested, so the same file answered the same question both ways. The visible case is "this rule set holds mangle rules only": the whole policy of such a rule set was installed in the filter table instead of the mangle table, where it decided nothing about packet marking and duplicated the filtering.

* Compiler (iptables): the name of a rule set, a branch or an interface is checked before it reaches the activation script. Both names go into the generated commands as plain words, and iptables accepts every character the kernel does, so a name holding shell syntax ran as a command on the firewall, as root, at the moment every chain is already set to drop. A name holding a wildcard in the middle was expanded against the directory the script runs from instead. Such a name is now reported and the rules using it are left out - which now also holds for the names iptables itself refuses, a rule set longer than 28 characters or named after an iptables target: those rules were reported and then written into the script anyway, where the command creating the chain fails and stops the activation with every rule behind it uninstalled. nftables filtered all of this already.
...
* Compiler (iptables, nftables): a rule limited to a calendar window keeps that window. Only the time of day and the weekdays of a time object were compiled, so a rule written to run from one date to another ran every day and never stopped. iptables gets `--datestart` / `--datestop` and nftables `meta time`; a firewall pinned below iptables 1.4.0, whose time match has no date options, is told what it loses. A window naming a year no packet filter can express is reported and the rule is left out.

* Compiler (iptables, nftables): a warning about a rule is written once instead of once per internal copy of that rule, and a warning raised while the compiler is only rehearsing a rule it then drops is no longer written at all. Errors were handled that way already; warnings were not, so nearly half of them were repeats and the report they belong to was hard to read.

* Compiler (iptables, nftables): a time object that names its weekdays as a first and a last day, the way Firewall Builder wrote them before it grew an explicit day list, keeps that restriction. The list was read as "every day" when it was empty, so a rule written for Monday to Friday fired at the weekend too.

* Compiler (iptables): a rule with the Branch action pointing at a rule set the firewall does not compile is reported. The generated script created the chain, jumped into it and never filled it, so the packet returned and the rule did nothing while the activation reported success. This covers a branch into the firewall's own top rule set, whose rules live in the built-in chains, and a branch into a rule set of another firewall object, which is not compiled into this script ([#90](https://github.com/Linuxfabrik/firewallfabrik/issues/90)). nftables reported both already.
...
* Compiler (iptables, nftables): a firewall with "Always permit SSH access from the management workstation" installs that rule in its ruleset. It only existed in the `block` and `stop` commands, so activating a policy that does not itself permit SSH cut the very session that was activating it and left the firewall unreachable - which is the one thing the option exists to prevent. The rule goes into the ruleset of the address family the management address belongs to. A firewall that does not compile that family, and a setting holding a host name rather than an address, are reported: a name is resolved when the ruleset is loaded, so it would cost the whole ruleset if it answered with more than one address. The `block` and `stop` commands keep taking a name.

* Compiler (nftables): a rule whose rate limit shares a table name with another rule that keys it differently is left out instead of being installed. Neither of the two then limited what it says it limits.
* Compiler (nftables): a rule whose source or destination is "not these addresses" now also covers traffic the firewall itself sends or receives, the way iptables always compiled it.
* Compiler (nftables): a NAT rule matching on a MAC address is compiled where the packet still carries one, and the script no longer claims it dropped a MAC match it kept.
...
* Compiler (iptables, nftables): a rule written for "any interface except these" no longer installs a rule on an unprotected interface, on a bridge port of a firewall that does not bridge, or on a cluster interface. Such a rule cannot match, so the traffic the rule was written to cover passed.
* Compiler (iptables, nftables): a rule from the firewall to itself no longer names the addresses of its bridge ports. Such an address belongs to the bridge, so the rule covered traffic the port only carries and never terminates.
* Compiler (nftables): a rule whose service is negated now covers the other protocols too, the way iptables always compiled it. "Anything but TCP port 80" was compiled as a TCP-only rule, so a Deny written that way let every UDP and ICMP packet through, and a NAT rule written that way let them cross untranslated.
* Compiler (nftables): a rule whose service is "anything but all TCP" or "anything but all UDP" no longer compiles into the rule it is the negation of. The negation was dropped, so a Deny written to stop everything except TCP stopped nothing but TCP.
* Compiler (nftables): a rule reading "from the firewall to anywhere but the firewall" is no longer compiled as if both ends were the firewall. It picked up the loopback address on both ends, so it matched traffic from the loopback and skipped traffic to it. iptables always read such a rule correctly.
...
* Compiler (iptables, nftables): the check for rules hidden by an earlier rule looks only at the rules of the table it is checking. On iptables it reported every finding twice, and it compared rules that end up in different tables and can never hide each other.
* Compiler (iptables): a time-restricted rule checks the time once instead of in every rule the optimizer generates for it, and a rule naming several time windows no longer multiplies the whole set of rules by the number of windows.
* Compiler (iptables): a logged rule with a connection limit or a rate limit kept per source, destination or port enforces that limit once instead of three times, so it admits the traffic the editor says it admits.
* Compiler (iptables, nftables): a rule branching into a rule set that marks packets or assigns a traffic class reaches those rules. The branch was installed only where the marking rules are not, so it jumped into an empty chain and nothing downstream saw the traffic the policy says is marked. Where the traffic class cannot be assigned at all - iptables refuses it before the routing decision - the branch is left out there and says so, instead of producing a command that stops the activation.
* Compiler (iptables, nftables): a rule to or from the firewall object keeps naming the firewall's addresses when the script also adds addresses for NAT. Those belong to a translation and not to the firewall, and the rule was shortened to one with no address at all, so "permit SSH to the firewall" permitted SSH to every translated address as well.
* Compiler (iptables, nftables): a rule to or from a bridging firewall's own address also covers the bridged path. Only the rule for traffic arriving on a routing interface was generated, so a rule permitting SSH to the firewall dropped it whenever it came in over a bridge.
* Compiler (iptables, nftables): a NAT rule that cannot be translated the way it is written is reported and left out. A one-to-one map between two networks of different sizes put two addresses on one, and a rule naming more than one translated service silently translated to the first of them.
* Compiler (iptables, nftables): a source translation whose address belongs to no interface of the firewall is written once for all interfaces of a kind instead of once per interface, and no longer names an unnumbered interface, a bridge port or an interface of the other address family, none of which the translated traffic can leave by.
* Compiler (iptables): a rule on a bridge port of a bridging firewall no longer produces a command iptables refuses, which stopped the activation with every chain already set to drop. A rule assigning a traffic class to traffic from or to anywhere now also covers forwarded traffic, which it could not reach before.
* Compiler (iptables, nftables): a NAT rule with the Branch action says that the translated addresses and service in the editor are not compiled. A branch jumps into another rule set and translates nothing of its own.
* Compiler (iptables, nftables): a dual-stack firewall no longer reports every single-stack rule it has. Such a rule is compiled for the family it names and left out of the other one, which is what it says it wants; the report said so once per rule and buried the messages that matter. A rule left out of the only family a rule set is compiled for is still reported.
* Compiler (iptables, nftables): the message about an interface configured with address 0.0.0.0 says what to do about it.
* Compiler (iptables, nftables): the message about a rule hidden by an earlier one names the destination and service it is about. The check compares every combination of source, destination and service separately, so a rule naming five services of which one is covered read like a rule that is dead - and deleting it took the other four with it.
...
* Compiler (iptables, nftables), Editor: the "MAC address matching" box of a host takes effect. It was stored under a name no compiler reads, so a host meant to be pinned to its MAC address was matched on its IP address alone, and the box showed unticked for every host imported from a `.fwb`.
...
* Compiler (nftables): the generated activation script answers an unknown command with the usage line and exit code 1, the way the iptables script does. It ended in a shell error instead, because no branch had set an exit code.
* Compiler (iptables, nftables): the `--xr` option traces a routing rule through the compiler, the way `--xp` and `--xn` trace a policy and a NAT rule. It was accepted and then ignored.
* Compiler (nftables): a rate limit kept per source, destination or port bounds how many of them it holds, from the same setting iptables uses. Without the bound the table grew until it was full and then stopped limiting anything it had not seen yet.
* Compiler (nftables): two rule options iptables reads and nftables cannot are reported instead of being dropped in silence. A NAT rule asking for "Use SNAT instead of MASQUERADE" on an interface whose address is only known at run time masquerades, and a rate limit asking for the "dstlimit" variant is compiled as the limit that replaced it.
* Compiler (nftables): a log prefix longer than the 127 characters nftables can carry is reported instead of being cut silently. A log parser reading the fields behind the cut was given no reason for their disappearance; iptables named its own shorter limits already.
* Compiler (nftables): a firewall reading an address table from a file checks that it can read it before it replaces its running ruleset, the way iptables already does. The file was only opened afterwards, so an unreadable one left the new ruleset installed with an empty set - a set no packet is in, which turns a Deny rule into one that blocks nothing - and ended the script without running the epilog.
...
* Compiler (iptables, nftables): a message about a rule is written once instead of once per internal copy of it. One rule as the editor shows it becomes several on the way to the script, and each of them repeated the same sentence, which buried the rest of the report.
* Compiler (iptables, nftables): a NAT rule whose Original Service lists a TCP service that inspects the flags next to a second service keeps the flag condition. The flags were dropped as soon as the rule named more than one service, so the rule translated every TCP packet on those ports instead of the handshake stage it was written for.
...
* Editor: a rule with the Tag action keeps its Tag Service when the file is saved and opened again. The reference was stored in a form that means nothing outside the session that wrote it, so every tagging rule of a saved policy was reported as having no mark and left out of the compiled script.
* Compiler (nftables): a NAT rule that names an unnumbered interface or a bridge port next to other objects still translates for those other objects. The whole rule was left out, where iptables drops just the object that carries no address.
* Compiler (iptables, nftables): a firewall asking for its bonding or VLAN interfaces to be configured is told that the generated script does not do that. It was ignored without a word, so the script came up without the interfaces its rules are written for ([#95](https://github.com/Linuxfabrik/firewallfabrik/issues/95)).
* Compiler (iptables, nftables): a tagging rule from a policy written with an older Firewall Builder, which stores the mark on the rule instead of in a Tag Service object, is compiled instead of being reported and left out. Nothing downstream that keys on the mark saw the traffic the policy says is marked.
* Compiler (iptables, nftables): a compile that the compiler refuses outright now says so and exits non-zero. It wrote no script and reported nothing, so a scripted or scheduled run counted the firewall as compiled and left the old script in place. Warnings about a firewall setting the compiler does not read arrive the same way.
* Compiler (iptables): a firewall activating through `iptables-restore` keeps the comment written on each rule and everything the compiler reported about it in the generated script. Only the rule number was written, so the script gave no reason for a rule looking the way it does.
* Compiler (iptables): compiling the same firewall twice in one session produces the same script again. The temporary chains a NAT rule needs were numbered from a counter that never started over, so every recompile renamed them and a diff against the deployed script showed changes nobody made.
...
* Compiler (iptables, nftables): two routing rules aiming at the same destination with the same metric install one route with several next hops instead of two competing ones. The second `ip route add` was refused with "File exists", so only one of the two paths was ever there and the activation reported an error. Two rules that also agree on the gateway and the interface are the same rule twice and the second is left out with a warning; two that differ only in the metric are reported, because which one the firewall should install cannot be guessed.
* Compiler (iptables, nftables): a NAT rule that translates the source *and* the destination, and translates both port numbers while doing it, translates the source again. Such a rule is compiled as a destination translation followed by a source translation, and the second one matched on the translated source port as well - a port the first rule can never write - so it matched nothing and the source was left untranslated.
* Compiler (iptables, nftables): a rule naming a host or firewall whose address comes from DHCP or PPP matches that address again. The placeholder 0.0.0.0 that Firewall Builder stores under such an interface went into the rule, and iptables reads a bare `0.0.0.0` as `0.0.0.0/32`, which no packet carries - so an anti-spoofing rule written about the firewall's own dynamic address never matched it. The rule now becomes the run-time loop over the interface's addresses, the way a rule naming the interface directly always did.
* Compiler (iptables, nftables): a routing rule whose gateway is on none of the local networks of the firewall, or not on the network of the interface the rule routes through, is reported and left out. The kernel refuses such a route with "Nexthop has invalid gateway", so it showed up as one line of error output in the middle of the activation and the route was simply not installed.
* Compiler (iptables, nftables): a firewall whose Policy or NAT rule sets are all branches is reported. Only the top rule set goes into the built-in chains, so such a firewall compiled to a script that installs no filtering at all and reported success. Firewall Builder has always said "Missing top level Policy ruleset" here.
* Compiler (iptables, nftables): a rule of a branch rule set that names a direction but no interface says which direction it means. The chain of a branch rule set is reached from the input *and* the output chain, so an "Outbound" rule there also applied to incoming traffic - an Accept written for one direction opened both. iptables gets `-i +` / `-o +`, the way Firewall Builder has always written it; nftables asks for an incoming or outgoing device index instead, because it refuses an all-wildcard interface name.
* Compiler (iptables, nftables): a rule naming a broadcast or a multicast address is installed on the paths such traffic actually takes - into and out of the firewall itself - instead of on the forwarding path, where it can never match. A rule permitting the firewall's own broadcasts, and one permitting a routing protocol that talks to a multicast group, therefore did nothing, while a Deny written the same way stopped nothing.
* Compiler (iptables, nftables): a rule naming an address the firewall carries is compiled as a rule about the firewall, even when the address is written as an object of its own rather than taken from the interface. Such a rule went into the forwarding path instead of into and out of the firewall, so it covered none of the traffic it was written for. The same now holds for the network and the broadcast address of an interface's subnet - a DHCP reply addressed to either is delivered locally.
* Compiler (iptables, nftables): a rule whose TCP or UDP service names a port range that runs backwards is reported and left out. iptables refuses the command outright, which stops the activation script with every chain already set to drop, and nftables refuses the whole ruleset, so the firewall never gets the new policy. The editor has always corrected such a range; a data file written elsewhere carries it.
* Compiler (iptables, nftables): a rule naming an address range whose end is below its start is reported and left out. iptables installed a rule that can never match, so a Deny written that way stopped nothing, and nftables refused the whole ruleset, so the firewall never got the new policy. Routing rules are checked the same way.
* Compiler (iptables): the name of an address table matched through ipset is shortened so the two helper sets built from it still fit, and every character the generated script cannot pass on is replaced. A name longer than 27 characters left the set empty, so a rule matching that list matched nothing and a block list blocked nobody; a name holding shell syntax ran as a command on the firewall, as root.
* Compiler (iptables, nftables): a rule whose TCP or UDP service names a port outside the range a port number has is reported and left out. iptables refuses the command and nftables the whole ruleset, so the firewall never got the new policy.
* Compiler (iptables, nftables): a bridging firewall no longer treats a broadcast or multicast address as its own when it decides which chain a rule goes into. A bridge forwards such a frame, so a rule about it belongs on the forwarding path as well, and Firewall Builder has always read it that way.
* Compiler (iptables, nftables): a rule naming a network the firewall itself is on also covers the traffic the firewall sends and receives on it. Only a network that contains one of the firewall's own addresses was recognised, so a segment carved out of the same LAN was read as a network somewhere else and the rule was compiled for forwarded traffic alone.
* Compiler (iptables, nftables): the kernel-hardening settings take effect. Reverse path filtering, ICMP redirects, source routing and martian logging exist once per interface next to the machine-wide copy, and the kernel combines the two rather than reading the machine-wide one on its own - so switching reverse path filtering or ICMP redirects *off* did nothing on any current distribution. The generated script now writes every copy.
* Compiler (iptables, nftables): the MAC address of a physical address object is checked before it reaches the activation script. Nothing had ever checked it, so a typo went into the generated rules as it was typed: iptables refuses the command outright, which stops the activation with every chain already set to drop, and nftables refuses the whole ruleset, so the firewall never gets the new policy. On iptables the value is a plain word in the generated script on top, so a MAC holding shell syntax ran as a command on the firewall, as root, at that moment. Such a rule is now reported and left out. A MAC written with dashes instead of colons - a spelling only nftables reads - is written out with colons for both platforms.
* Compiler (iptables, nftables): a rule naming a host or a second firewall object that stands for the firewall itself is compiled as a rule about the firewall. Only the firewall object the compile runs for was recognised, so a machine modelled twice - as a Host in another library, or as a second Firewall object - was treated as some other machine and its rules went onto the forwarding path, where the firewall's own traffic never goes. The same now holds for a host standing for a broadcast or a multicast group.
* Compiler (iptables, nftables): a rule whose source is a host matched by IP *and* MAC address keeps matching on the IP address where the packet carries no ethernet header any more. Such a rule was left out entirely, so a rule the administrator wrote disappeared from the ruleset - fail-closed on an Accept and fail-open on a Deny. Only a rule that names nothing but a MAC still goes, because there is nothing left to match on.
* Compiler (iptables, nftables): a rule naming an interface of another host or firewall that carries one of this firewall's own addresses is compiled as a rule about the firewall, and a NAT rule finds the interface facing an address the same way the rest of the compiler does. Both questions were answered from the object tree alone, so the same address described from the other end was read as some other machine and the rule went onto the forwarding path.
* Compiler (nftables): a rule written outbound whose destination is a network - a multicast group, for instance - is no longer thrown away without a word. The check that removes an outbound rule the firewall itself would never see had three of its four exceptions missing on nftables, so a rule such as "permit OSPF out of eth1 to ff00::/8" disappeared from the nftables ruleset while the iptables one kept it. iptables was already right.
...
* GUI: File > Import Library works. It failed with an error message and imported nothing, and it skipped every library whose name is already in use - which is every ordinary data file, because they all call their library "User". Such a library is imported under a free name now, the way Firewall Builder does it. The file's own Standard library comes along as an editable copy, because the two files can carry different generations of it and the imported rules would otherwise lose every service they name there and match every service instead ([#150](https://github.com/Linuxfabrik/firewallfabrik/issues/150)).
* GUI: deleting an object disables the rules whose last source, destination or service it was, instead of leaving them matching everything. A rule written for one host became a rule for every host the moment that host was deleted, and nothing said so.
