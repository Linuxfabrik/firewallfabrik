...
* Compiler (nftables): a rule whose service element is negated and names a service inspecting TCP flags keeps its negation - the flagged service was given a rule of its own, and the rule left beside it matched every packet.
* Compiler (nftables): a rule whose service element is negated and names a service restricted to a source *and* a destination port keeps its negation - the two ports were excluded separately, which asks for a packet on neither and let through what the rule was written to stop.
