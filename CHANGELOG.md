...
* Compiler (iptables): a dual-stack firewall pinned to an older iptables no longer gets IPv6 rules with matches that release only had for IPv4. The address-range, ToS, DiffServ and address-table matches reached ip6tables between 1.4.0 and 1.4.9; before that the generated ip6tables command failed to load and stopped the activation script, leaving every rule after it uninstalled. A firewall pinned before 1.3.7 logs through LOG instead of NFLOG for the same reason.
...
* Compiler (iptables, nftables): a log prefix holding a double quote or a control character is reported and written without it, instead of breaking the nftables ruleset and silently losing the quotes on iptables.
