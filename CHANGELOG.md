...
* Compiler (nftables): a rule whose service is negated now covers the other protocols too, the way iptables always compiled it. "Anything but TCP port 80" was compiled as a TCP-only rule, so a Deny written that way let every UDP and ICMP packet through.
