...
* REJECT rules for IPv6 policies now emit the correct `icmp6-*` reject types (`icmp6-port-unreachable`, `icmp6-addr-unreachable`, `icmp6-adm-prohibited`) instead of the IPv4 `icmp-*` names. The previous output was rejected by `ip6tables-restore` and the IPv6 script failed to load on the firewall
