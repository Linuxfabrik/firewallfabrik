...
* Compiler (iptables) now expands a firewall object that remains in a rule's source or destination after chain assignment to one rule per non-loopback interface address, matching fwbuilder. Anti-spoofing rules with the firewall itself as source therefore now generate one drop rule per own IP, not just one for the address of the rule's own interface. Previously the print stage resolved the firewall object to the first non-loopback address it found, so all other own IPs were silently missed -- a spoofed packet from another own interface address would not have been blocked
...
* Compiler (iptables) no longer writes `echo 0 > /proc/sys/net/ipv4/tcp_fin_timeout` or `echo 0 > /proc/sys/net/ipv4/tcp_keepalive_intvl` into the generated script when the firewall option is left at its default ("kernel default"). Firewall Builder represented "not set" for these two options as the integer `0`; fwf was honouring that literal `0` and pushing it into the kernel, which would have closed TIME_WAIT sockets instantly and disabled TCP keepalive probe spacing. Both values are now skipped exactly as fwbuilder itself skips them, matching `OSConfigurator_linux24.cpp`
* Compiler (iptables) temporary chain names are now stable across repeated compilations. The chain identifier is derived from the rule set name, rule position and subrule suffix instead of the Rule's internal UUID (which is regenerated on every `.fwb` / `.fwf` load). Consequence: recompiling an unchanged policy now produces byte-identical iptables scripts, so CI/CD pipelines, checksums and `diff` / `git diff` no longer see phantom changes. Compiling the same policy from a `.fwb` source and from its saved `.fwf` counterpart now also produces byte-identical output
...
* Compiling a firewall whose outbound rules target an AddressRange object (e.g. a custom-defined subnet) no longer aborts the iptables compiler with `'AddressRange' object has no attribute 'is_multicast'`
...
* Compiler no longer reports spurious "Rule X shadows Rule Y below it" warnings for rules whose source or destination is "any". Both iptables and nftables compilers previously ran shadow detection after the rule had been split into synthetic INPUT and OUTPUT copies targeting the firewall itself; the synthetic copies looked identical to legitimate firewall-to-firewall rules above, so every such pair triggered a false-positive shadow warning. Shadow detection now runs before those chain-assignment splits (matching fwbuilder's behaviour) and additionally skips pairs whose atomic match content is identical
