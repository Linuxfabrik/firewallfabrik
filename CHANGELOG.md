...
* Compiler (iptables) now emits numeric syslog levels (e.g. `--log-level 5`) instead of symbolic names (`--log-level notice`) on LOG rules, matching fwbuilder output. Both forms are functionally identical but the numeric form is what every iptables version understands out of the box
