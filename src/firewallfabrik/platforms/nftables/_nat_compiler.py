        ...
        for slot in ('osrc', 'odst', 'osrv', 'tsrc', 'tdst', 'tsrv'):
            for obj in getattr(rule, slot):
                ...
                if data.get('rule_error', False):
                    ...
                    # named.
                    self.compiler.abort(
                        rule, f"Object '{name}' has errors. The rule is left out"
                    )
                    return True
        self.tmp_queue.append(rule)
        return True
...
class ClassifyNATRule(NATRuleProcessor):
    ...
    def process_next(self) -> bool:
        ...
        # A branch jumps into another rule set and translates nothing
        # itself, so whatever the editor left in the translated elements is
        # not compiled - and saying so beats leaving it there for a later
        # processor to read (NATCompiler::classifyNATRule).
...
class AssignInterface(NATRuleProcessor):
    ...
    def process_next(self) -> bool:
        ...
        # `oifname "eth*"` instead of four rules.
