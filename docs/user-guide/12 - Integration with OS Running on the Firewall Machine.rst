...
Coexistence with Docker, CrowdSec, fail2ban and Other Tools (nftables)
-----------------------------------------------------------------------

On servers running Docker, CrowdSec, fail2ban or similar tools, each of these tools manages its own nftables tables and chains. FirewallFabrik's nftables compiler uses **named tables** so that it never touches rules created by other tools.

By default, FirewallFabrik creates two tables:

* ``linuxfabrik_filter`` — packet filtering rules (input, forward, output chains)
* ``linuxfabrik_nat`` — NAT rules (prerouting, postrouting, output chains)

When the generated script starts, stops or reloads, it only deletes and recreates these two tables. All other nftables tables (e.g. Docker's ``table ip docker`` or CrowdSec's ``table inet crowdsec``) remain untouched.

You can change the base table name in the nftables settings dialog (``Firewall Settings > Compiler > Table name``). The default is ``linuxfabrik``, which produces the table names ``linuxfabrik_filter`` and ``linuxfabrik_nat``.

.. note::

   This coexistence only applies to the **nftables** platform. The iptables compiler still performs a full flush of all iptables chains before loading the new rules, because iptables does not offer a comparable namespace mechanism. If you need coexistence with Docker or other tools, use the nftables platform.
