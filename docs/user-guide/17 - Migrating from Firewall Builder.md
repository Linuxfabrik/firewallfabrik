...
The `use_numeric_log_levels` firewall option is honoured by the iptables compiler: when enabled, LOG rules are emitted with `--log-level 5` instead of `--log-level notice`, matching the form every iptables version understands out of the box. The option has no effect on nftables: the nft grammar accepts only the nine level keywords `emerg`, `alert`, `crit`, `err`, `warn`, `notice`, `info`, `debug` and `audit`, so a numeric level is a parse error. Its checkbox is therefore greyed out in the nftables firewall settings dialog.

Legacy and iptables-only options  
The iptables firewall settings dialog exposes advanced options such as logging TCP sequence numbers, logging TCP/IP options, unconditional rule logging, and bridge interface configuration. These are implemented by the iptables compiler. One option, the management SSH rule in the block/stop action (`add_mgmt_ssh_rule_when_stoped`), is not implemented, and its checkbox is disabled. The nftables firewall settings dialog does not show the options that describe an iptables implementation detail with no nftables counterpart at all: loading netfilter kernel modules (`load_modules`), the ipset module toggle (`use_m_set`), and `iptables-restore` activation (`use_iptables_restore`). nft loads the required modules on demand, provides named sets as a native replacement for ipset, and always loads the ruleset in one atomic transaction. Options that do apply to nftables but that the compiler does not use are shown greyed out, with a tooltip stating the reason. A value your `.fwb` file had stored for one of the three hidden options is kept as it is, so switching the firewall back to iptables restores it.
