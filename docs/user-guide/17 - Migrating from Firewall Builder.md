...
The `use_numeric_log_levels` firewall option is honoured by the iptables compiler: when enabled, LOG rules are emitted with `--log-level 5` instead of `--log-level notice`, matching the form every iptables version understands out of the box. The nftables compiler does not yet honour this option.

Legacy and iptables-only options  
The iptables firewall settings dialog exposes advanced options such as logging TCP sequence numbers, logging TCP/IP options, using the kernel timezone for time-based rules, unconditional rule logging, and bridge interface configuration. These are implemented by the iptables compiler. One option, the management SSH rule in the block/stop action (`add_mgmt_ssh_rule_when_stoped`), is not yet implemented, and its checkbox is disabled. In the nftables firewall settings dialog, options that are specific to the iptables backend (for example the kernel-timezone switch, the ipset module toggle, and `iptables-restore` activation) are not shown, because nftables either provides native equivalents (for example native sets in place of the ipset kernel module) or does not need them. If your `.fwb` file had an unsupported option enabled, you will see a compiler warning.
